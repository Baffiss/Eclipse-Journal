...
const AnalyticsPage: React.FC<{ isComponent?: boolean; defaultAccountId?: string; defaultStrategyId?: string }> = ({
  ...
  const equityCurveData = useMemo(() => {
    const activeAccountIds = new Set(accounts.filter(a => a.status === AccountStatus.ACTIVE).map(a => a.id));
    const equityTrades = filteredTrades.filter(t => activeAccountIds.has(t.accountId));
    const equityWithdrawals = withdrawals.filter(w => activeAccountIds.has(w.accountId));
  ...
  if (filteredTrades.length === 0) {
    return (
      <div className={`flex flex-col items-center justify-center py-32 bg-muted/10 border-2 border-dashed border-border rounded-[3rem] animate-fade-in`}>
        ...
        <h3 className={`text-2xl font-black tracking-tight uppercase`}>{t('notEnoughData')}</h3>
        <p className={`text-muted-foreground font-black mt-2 max-w-sm text-center uppercase text-[10px] tracking-widest`}>{t('selectAccountOrStrategyForAnalytics')}</p>
      </div>
    );
  }
    ...
    <div className={`animate-fade-in flex flex-col gap-8 pb-20`}>
      {!isComponent && (
        <div className={`flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-muted/30 backdrop-blur-xl border border-border rounded-[2.5rem] p-6 lg:p-8`}>
          <div>
            <h1 className={`text-4xl font-black tracking-tighter leading-none uppercase`}>{t('analytics')}</h1>
          </div>
          <div className={`flex flex-wrap gap-4 w-full lg:w-auto`}>
            <div className={`relative group flex-1 lg:w-56`}>
              <LayoutGridIcon className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground group-focus-within:text-primary transition-colors pointer-events-none z-10`} />
              <select
            ...
            <div className={`relative group flex-1 lg:w-56`}>
              <TargetIcon className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground group-focus-within:text-primary transition-colors pointer-events-none z-10`} />
              <select
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-6`}>
        <DataTile label="Total P&L" value={`${currencySymbol}${stats.totalProfit.toLocaleString()}`} color={stats.totalProfit >= 0 ? 'success' : 'danger'} icon={<ZapIcon />} description={t('totalProfit_desc')} />
        <DataTile label="Win Rate" value={`${stats.winRate.toFixed(1)}%`} color="primary" icon={<TargetIcon />} description={t('winRate_desc')} extra={<div className={`w-16 h-16`}><ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}><PieChart><Pie data={winRatePieData} innerRadius={18} outerRadius={28} dataKey="value" stroke="none" cx="50%" cy="50%">{winRatePieData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}</Pie></PieChart></ResponsiveContainer></div>} />
        <DataTile label="Profit Factor" value={stats.profitFactor?.toFixed(2) || '0.00'} color={Number(stats.profitFactor) > 1.5 ? 'success' : 'primary'} icon={<ActivityIcon />} description={t('profitFactor_desc')} />
        <DataTile label="Avg. R:R" value={stats.payoffRatio ? `1:${stats.payoffRatio.toFixed(2)}` : 'N/A'} color="primary" icon={<TrendingUpIcon />} description={t('payoffRatio_desc')} />
        <DataTile label="Expected Value" value={`${currencySymbol}${stats.expectedValue.toFixed(2)}`} color={stats.expectedValue >= 0 ? 'success' : 'danger'} icon={<BanknoteIcon />} description={t('expectedValue_desc')} />
        <DataTile label="Max Drawdown" value={`${stats.maxDrawdown.toFixed(1)}%`} color="danger" icon={<TrendingDownIcon />} description={t('maxDrawdown_desc')} />
      </div>

      <div className={`grid grid-cols-1 ${isComponent ? '' : 'xl:grid-cols-2'} gap-8`}>
        {!isComponent && (
          <div className={`bg-muted/10 border border-border rounded-[3rem] p-8 lg:p-10 min-h-[480px] flex flex-col relative`}>
            <div className={`flex justify-between items-center mb-8`}>
              <h3 className={`text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-1`}>Equity Curve (Active Only)</h3>
              <div className={`p-3 bg-muted/20 rounded-2xl border border-border`}><ActivityIcon className={`w-5 h-5 text-primary`} /></div>
            </div>
            <div className={`flex-1 min-h-[350px]`}><EquityChart data={equityCurveData} currencySymbol={currencySymbol} account={activeAccount} /></div>
          </div>
        )}
        <div className={`bg-muted/10 border border-border rounded-[3rem] p-8 lg:p-10 min-h-[480px] flex flex-col relative ${isComponent ? 'xl:col-span-1' : ''}`}>
          <div className={`flex justify-between items-center mb-8`}>
            <div>
            ...
            <div className={`p-3 bg-muted/20 rounded-2xl border border-border`}><TrendingUpIcon className={`w-5 h-5 text-primary`} /></div>
          </div>
          <div className={`flex-1 min-h-[350px]`}>
            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
              <AreaChart data={totalProfitCurveData} margin={{ top: 10, right: 50, left: 10, bottom: 10 }}>
                <defs><linearGradient id="colorProfit" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={`hsl(var(--success))`} stopOpacity={0.15}/><stop offset="95%" stopColor={`hsl(var(--success))`} stopOpacity={0}/></linearGradient></defs>
      ...
      <div className={`grid grid-cols-1 xl:grid-cols-3 gap-8`}>
        ...
        <div className={`bg-muted/20 border border-border rounded-[3rem] p-10 flex flex-col md:flex-row gap-8 items-center xl:col-span-1 min-h-[400px]`}>
          <div className={`flex-1`}><h3 className={`text-[11px] font-black uppercase tracking-[0.3em] text-primary mb-2`}>Alpha Dominance</h3><p className={`text-muted-foreground font-black text-[10px] uppercase tracking-widest mb-8`}>Identification of top performing assets.</p><div className={`space-y-4`}>{stats.assetPerformance.slice(0, 3).map((asset, i) => (<div key={asset.asset} className={`flex justify-between items-center p-4 bg-bkg border border-border rounded-2xl shadow-sm`}><div className={`flex items-center gap-3`}><div className={`w-2.5 h-2.5 rounded-full`} style={{ backgroundColor: PIE_CHART_COLORS[i] }} /><span className={`text-sm font-black uppercase tracking-tight`}>{asset.asset}</span></div><span className={`text-sm font-black text-success`}>+{currencySymbol}{asset.profit.toLocaleString()}</span></div>))}</div></div>
        ...
        <div className={`space-y-6 xl:col-span-1 min-h-[400px]`}><div className={`group bg-success/10 border border-success/30 rounded-[2.5rem] p-8 hover:bg-success/15 transition-all duration-300`}><div className={`flex justify-between items-center mb-6`}><div className={`p-3 bg-success/20 text-success rounded-2xl border border-success/30`}><ArrowUpRightIcon className={`w-6 h-6`} /></div><span className={`text-[10px] font-black uppercase tracking-[0.3em] text-success`}>Peak Execution</span></div><div className={`flex justify-between items-end`}><div><h4 className={`text-3xl font-black tracking-tighter uppercase mb-1`}>{stats.maxWin?.asset}</h4><p className={`text-xs font-black text-muted-foreground uppercase tracking-widest`}>{new Date(stats.maxWin?.date || '').toLocaleDateString()}</p></div><div className={`text-right`}><p className={`text-2xl font-black text-success`}>+{currencySymbol}{stats.maxWin?.result.toLocaleString()}</p></div></div></div><div className={`group bg-danger/10 border border-danger/30 rounded-[2.5rem] p-8 hover:bg-danger/15 transition-all duration-300`}><div className={`flex justify-between items-center mb-6`}><div className={`p-3 bg-danger/20 text-danger rounded-2xl border border-danger/30`}><ArrowDownRightIcon className={`w-6 h-6`} /></div><span className={`text-[10px] font-black uppercase tracking-[0.3em] text-danger`}>Bottom Out</span></div><div className={`flex justify-between items-end`}><div><h4 className={`text-3xl font-black tracking-tighter uppercase mb-1`}>{stats.maxLoss?.asset}</h4><p className={`text-xs font-black text-muted-foreground uppercase tracking-widest`}>{new Date(stats.maxLoss?.date || '').toLocaleDateString()}</p></div><div className={`text-right`}><p className={`text-2xl font-black text-danger`}>{currencySymbol}{stats.maxLoss?.result.toLocaleString()}</p></div></div></div></div>
      </div>
