...
// Importance values for widget filtering (0=High, 1=Medium, 2=Low)
...
const TradingViewWidget: React.FC<WidgetProps> = ({ src, config, className, style }) => {
    ...
    useEffect(() => {
        ...
        // Clean up previous widget content thoroughly
        ...
        script.src = src;
        script.async = true;
        // TradingView widgets read their config from the script tag's inner content
...
const EconomicCalendar = memo(({ theme, locale, currencies, importance }: EconomicCalendarProps) => {
    const config = {
        ...
        "isTransparent": true,
        "locale": locale,
        "importanceFilter": importance.length > 0 ? importance.join(",") : "0,1,2",
        "currencyFilter": currencies.length > 0 ? currencies.join(",") : undefined
    };
                ...
                {isEditing ? (
                    <div className="p-4 h-full flex flex-col">
                        ...
                        <div className="flex-grow overflow-y-auto space-y-2 pr-1 scrollbar-thin">
                            ...
                            {symbols.length === 0 && <p className="text-center text-sm text-muted-foreground py-4">No symbols in watchlist</p>}
                        </div>
                    </div>
                ) : (
                    symbols.length > 0 ? (
                        ...
                        />
                            ...
                            No symbols added.<br/>Click the edit button to manage your watchlist.
                        </div>
                    )
                )}
            </div>
...
const MarketsPage: React.FC = () => {
    ...
    const [calendarFilters, setCalendarFilters] = useState({
        ...
        importance: IMPORTANCE_LEVELS.map(i => i.value)
    });
    ...
    const renderActiveWidget = () => {
        switch (activeTab) {
            case 'overview': return <MarketOverview key={`overview-${widgetTheme}-${widgetLocale}`} theme={widgetTheme} locale={widgetLocale} />;
            case 'forex': return <ForexHeatmap key={`heatmap-${widgetTheme}-${widgetLocale}`} theme={widgetTheme} locale={widgetLocale} />;
            case 'crypto': return <CryptoHeatmap key={`crypto-${widgetTheme}-${widgetLocale}`} theme={widgetTheme} locale={widgetLocale} />;
            case 'stocks': return <StockHeatmap key={`stocks-${widgetTheme}-${widgetLocale}`} theme={widgetTheme} locale={widgetLocale} />;
            default: return null;
        }
    };

    const tabs: { id: Tab, label: string, icon: React.ReactElement }[] = [
        { id: 'overview', label: t('marketOverview'), icon: <LayoutGridIcon className="w-4 h-4"/> },
        { id: 'forex', label: t('forexHeatmap'), icon: <BanknoteIcon className="w-4 h-4"/> },
        { id: 'crypto', label: t('cryptoHeatmap'), icon: <CpuIcon className="w-4 h-4"/> },
        { id: 'stocks', label: t('stockHeatmap'), icon: <TrendingUpIcon className="w-4 h-4"/> },
    ];

    return (
        <div className="animate-fade-in flex flex-col space-y-4 pb-8 min-h-screen">
            ...
            <div className="space-y-4 flex-shrink-0">
                ...
                <div className="w-full bg-bkg/50 backdrop-blur-md border border-border rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300" style={{ height: '72px' }}>
                    <TickerTape key={`ticker-${widgetTheme}-${widgetLocale}`} theme={widgetTheme} locale={widgetLocale} />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-4">
                ...
                {/* 1. Left Column (Main Charts & Calendar) - Spans 3 columns on XL */}
                <div className="xl:col-span-3 flex flex-col gap-4">
                    
                    {/* A. Main Visualization Tabs */}
                    <div className="bg-bkg/50 backdrop-blur-xl border border-border rounded-2xl shadow-sm flex flex-col h-[600px] overflow-hidden">
                        <div className="flex flex-col sm:flex-row justify-between items-center px-4 py-3 border-b border-border/50 gap-2 bg-transparent">
                            <h2 className="text-lg font-semibold flex items-center gap-2">
                            ...
                            <div className="flex bg-muted/50 rounded-lg p-1 gap-1 overflow-x-auto no-scrollbar max-w-full">
                                {tabs.map((tab) => (
                                    <button
                                        key={tab.id}
                                        onClick={() => setActiveTab(tab.id)}
                                        className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all duration-200 whitespace-nowrap flex items-center gap-1.5 ${
                                            activeTab === tab.id ? 'bg-bkg text-primary shadow-sm' : 'text-muted-foreground hover:bg-muted/80 hover:text-content'
                                        }`}
                                    >
                    ...
                    {/* B. Economic Calendar */}
                    <div className="bg-bkg/50 backdrop-blur-xl border border-border rounded-2xl shadow-sm flex flex-col h-[500px] overflow-hidden">
                        <div className="flex justify-between items-center px-4 py-3 border-b border-border/50 bg-transparent flex-shrink-0">
                            <h2 className="text-lg font-semibold flex items-center gap-2">
                                <CalendarIcon className="w-5 h-5 text-primary" />
                                {t('economicCalendar')}
                                <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded ml-2 hidden sm:inline-block">Powered by TradingView</span>
                            </h2>
                             <button 
                                onClick={() => setShowCalendarFilters(!showCalendarFilters)}
                                className={`p-2 rounded-md transition-all duration-200 ${showCalendarFilters ? 'bg-primary text-bkg shadow-sm' : 'text-muted-foreground hover:bg-muted'}`}
                        ...
                        <div className="flex-grow relative overflow-hidden">
                            <EconomicCalendar 
                                theme={widgetTheme} 
                                locale={widgetLocale} 
                                currencies={calendarFilters.currencies}
                                importance={calendarFilters.importance}
                            />
                        </div>
                    </div>
                ...
                {/* 2. Right Sidebar (Watchlist & News) - Spans 1 column on XL */}
                <div className="xl:col-span-1 flex flex-col gap-4">
                    ...
                    <div className="bg-bkg/50 backdrop-blur-xl border border-border rounded-2xl shadow-sm flex flex-col h-[500px] overflow-hidden">
                        ...
                        <div className="flex-grow relative overflow-hidden">
                            <MarketNews key={`news-${widgetTheme}-${widgetLocale}`} theme={widgetTheme} locale={widgetLocale} />
                        </div>
                    </div>
