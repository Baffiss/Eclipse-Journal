...
const WatchlistWidget = memo(({ theme, locale }: { theme: string, locale: string }) => {
    ...
    const config = { "width": "100%", "height": "100%", "symbolsGroups": [{ "name": t('watchlist'), "symbols": symbols.map(s => ({ "name": s })) }], "colorTheme": theme, "isTransparent": true, "locale": locale };
    ...
    return (
        <div className={`bg-bkg/50 backdrop-blur-xl border border-border rounded-2xl shadow-sm flex flex-col overflow-hidden transition-[height] duration-300 ease-in-out min-w-0`} style={{ height: `${containerHeight}px` }}>
            <div className={`flex justify-between items-center px-4 py-3 border-b border-border/50 bg-transparent flex-shrink-0`}>
                <h2 className={`text-lg font-semibold flex items-center gap-2 truncate`}><EyeIcon className={`w-5 h-5 text-primary`} /> {t('watchlist')}</h2>
                <button onClick={() => setIsEditing(!isEditing)} className={`p-1.5 rounded-md flex-shrink-0 transition-colors ${isEditing ? `bg-primary text-bkg` : `hover:bg-muted text-muted-foreground`}`} title={t('editWatchlist')}><EditIcon className={`w-4 h-4`} /></button>
            </div>
            ...
            <div className={`grid grid-cols-1 lg:grid-cols-4 gap-4 w-full`}>
                <div className={`lg:col-span-3 flex flex-col gap-4 min-w-0`}>
                    ...
                    <div className={`bg-bkg/50 backdrop-blur-xl border border-border rounded-2xl shadow-sm flex flex-col h-[500px] overflow-hidden min-w-0`}>
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ease-in-out bg-bkg border-b border-border/50 flex-shrink-0 ${showCalendarFilters ? `max-h-[600px] opacity-100 shadow-sm` : `max-h-0 opacity-0`}`}><div className={`p-4 space-y-4 overflow-y-auto max-h-[300px] scrollbar-thin`}><div><h3 className={`text-xs font-bold uppercase text-muted-foreground mb-2 flex items-center gap-2 tracking-wider`}><TrendingUpIcon className={`w-3 h-3`} /> {t('impact')}</h3><div className={`flex gap-2`}>{IMPORTANCE_LEVELS.map(level => (<button key={level.value} onClick={() => toggleImportance(level.value)} className={`flex-1 px-3 py-2 text-xs font-medium rounded-md transition-all ${calendarFilters.importance.includes(level.value) ? `bg-primary text-bkg border border-primary shadow-sm` : `bg-muted/30 border border-transparent text-muted-foreground hover:bg-muted`}`}>{level.label}</button>))}</div></div><div><div className={`flex justify-between items-center mb-2`}><h3 className={`text-xs font-bold uppercase text-muted-foreground flex items-center gap-2 tracking-wider`}><GlobeIcon className={`w-3 h-3`} /> {t('currencies')}</h3><div className={`flex gap-2`}><button onClick={selectAllCurrencies} className={`text-[10px] text-primary hover:underline`}>{t('all')}</button><button onClick={deselectAllCurrencies} className={`text-[10px] text-muted-foreground hover:underline`}>{t('clear')}</button></div></div><div className={`grid grid-cols-3 sm:grid-cols-4 gap-2`}>{AVAILABLE_CURRENCIES.map(curr => { const isSelected = calendarFilters.currencies.includes(curr.code); return (<button key={curr.code} onClick={() => toggleCurrency(curr.code)} className={`flex items-center justify-center gap-1.5 p-2 rounded-lg border text-xs font-medium transition-all ${isSelected ? `bg-primary text-bkg border-primary shadow-sm` : `bg-muted/30 border-transparent text-muted-foreground hover:bg-muted`}`}>{isSelected && <div className={`w-1.5 h-1.5 rounded-full bg-bkg`} />}{curr.code}</button>); })}</div></div></div></div>
