...
const AccountDetailView: React.FC<{ account: Account; onBack: () => void }> = ({ account, onBack }) => {
  ...
  useEffect(() => {
    if (!isWithdrawalEnabled && activeTab === 'withdrawals') {
      setActiveTab('trades');
    }
  }, [isWithdrawalEnabled, activeTab]);
    ...
    <div className={`animate-slide-in-up flex flex-col gap-10`}>
      ...
      <div className={`grid grid-cols-2 ${isWithdrawalEnabled ? 'md:grid-cols-3 lg:grid-cols-5' : 'md:grid-cols-2 lg:grid-cols-4'} gap-4 sm:gap-6`}>
        ...
        <div className={`bg-bkg border border-border rounded-[2rem] p-5 sm:p-6 shadow-sm overflow-hidden`}>
          ...
          <p className={`text-xl xl:text-2xl font-black tracking-tighter break-all ${profit >= 0 ? `text-success` : `text-danger`}`}>{profit >= 0 ? `+` : ``}{currencySymbol}{profit.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
          <span className={`text-[9px] font-black uppercase tracking-widest ${profit >= 0 ? `text-success` : `text-danger`} opacity-80`}>{profitPercent.toFixed(2)}% ROI</span>
        </div>
        {isWithdrawalEnabled && (
          <div className={`bg-bkg border border-border rounded-[2rem] p-5 sm:p-6 shadow-sm overflow-hidden`}>
        ...
        <div className={`bg-bkg border border-border rounded-[2rem] p-5 sm:p-6 shadow-sm overflow-hidden col-span-2 md:col-span-1`}>
          ...
          <p className={`text-xl xl:text-2xl font-black tracking-tighter break-all`}>{account.drawdownValue}{account.drawdownValueType === ValueType.PERCENTAGE ? `%` : ` ${currencySymbol}`}</p>
          <span className={`text-[9px] font-black uppercase tracking-widest text-muted-foreground opacity-60`}>{t(account.drawdownType.toLowerCase())}</span>
        </div>
      </div>
      ...
      <div className={`bg-bkg border border-border rounded-[3.5rem] overflow-hidden shadow-sm`}>
        <div className={`px-10 py-6 border-b border-border bg-muted/20 flex flex-col sm:flex-row justify-between items-center gap-4`}>
          <div className={`flex bg-muted/50 rounded-2xl p-1 gap-1`}>
            <button onClick={() => setActiveTab(`trades`)} className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'trades' ? 'bg-primary text-bkg shadow-lg' : 'text-muted-foreground hover:bg-muted'}`}>{t('recentTrades')}</button>
            {isWithdrawalEnabled && (
              <button onClick={() => setActiveTab(`withdrawals`)} className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'withdrawals' ? 'bg-primary text-bkg shadow-lg' : 'text-muted-foreground hover:bg-muted'}`}>{t('withdrawalLog')}</button>
            )}
          </div>
        ...
        <div className={`divide-y divide-border`}>
          {activeTab === 'trades' ? (
            accountTrades.length > 0 ? accountTrades.slice(0, 20).map(trade => <TradeListItem key={trade.id} trade={trade} currencySymbol={currencySymbol} />) : (<div className={`py-20 text-center opacity-40`}><TargetIcon className={`w-12 h-12 mx-auto mb-4`} /><p className={`text-sm font-black uppercase tracking-widest`}>{t('noTradesYet')}</p></div>)
          ) : isWithdrawalEnabled ? (
            accountWithdrawals.length > 0 ? accountWithdrawals.map(withdrawal => <WithdrawalListItem key={withdrawal.id} withdrawal={withdrawal} currencySymbol={currencySymbol} onDelete={() => deleteWithdrawal(withdrawal)} />) : (<div className={`py-20 text-center opacity-40`}><BanknoteIcon className={`w-12 h-12 mx-auto mb-4`} /><p className={`text-sm font-black uppercase tracking-widest`}>{t('noWithdrawalsYet')}</p></div>)
          ) : null}
        </div>
      </div>
